...
addpath ./netlab/[X,Yc,T,AUCo] = perfcurve(tn(:,Test_ix),Yhat_test,1)
pn = importdata('./NAremove_train.mat');
pn_name = importdata('./NAremove_train_colnames.mat');
tn = importdata('./NAremove_train_target.mat');
tn = double(tn);
n = size(pn,1);

rp = randperm(29072);
Test_ix = rp(1:5000);
Val_ix = rp(5001:10000);
Train_ix = setdiff(1:29072, Test_ix);
